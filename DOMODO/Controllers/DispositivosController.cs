...
namespace DOMODO.Controllers
{
    public class DispositivosController : Controller
    {
        ...
        public ActionResult DispositivosCentral(int id)
        {
            ...
            List<Peticion> listpeticion = db.Peticion.ToList();
            //realiza petición sensores
            foreach (var disp in list)
            {
                if (disp.Sensores != null) {
                    foreach (var sensor in disp.Sensores)
                    {
                        var exist = db.Peticion.Where(x => x.IdSensor == sensor.IdSensores && x.Estado == "pendiente" && x.Valor == "0" && x.Accion == "leer").FirstOrDefault();
                        if (exist == null) {
                            db.Peticion.Add(new Peticion { IdSensor = sensor.IdSensores, Estado = "pendiente", Valor = "0", Accion = "leer" });
                        }
                    }
                }
            }
        ...
        [HttpPost]
        public ActionResult Create([Bind(Include = "IdDispositivo,IdCentral,Nombre,Descripcion,Identificador")] Dispositivo dispositivo)
        {
        ...
        public ActionResult Edit([Bind(Include = "IdDispositivo,IdCentral,Nombre,Descripcion,Identificador")] Dispositivo dispositivo)
        {
            if (ModelState.IsValid)
            {
